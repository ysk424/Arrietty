...
2. VRを開始してコースの正面を向きます。
3. ペダルを数回踏み、CYCPLUS T2を起こします。
4. ハンドルを中央に保ったまま`Numpad 0`を押します。この時点で最初に受信したコントローラ姿勢が直進基準になります。
...
Version 0.5.0の操舵は初回カーブ試験用です。ホイールベースは1.05 m、実ハンドル角の35%を仮の前輪操舵角として使います。感度、ニュートラル、体感との一致は次の調整対象です。
...
Version 0.5.0 は VR セッション操作、テンキー調整、HMD正面方向への手動移動、CYCPLUS T2による速度入力、右VIVEコントローラによる暫定操舵を実装しています。コース自動追従、操舵感の最終調整、VIVE Tracker、地図、心拍連動はまだ実装していません。
